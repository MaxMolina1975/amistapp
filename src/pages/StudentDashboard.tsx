...
import { Gift, Shield, Users, Coins, Send, Heart } from 'lucide-react';
...
export function StudentDashboard() {
  ...
  const studentInfo = {
    ...
    nextReward: 200, // TODO: Obtener del sistema de recompensas
    monthlyBonus: currentUser?.monthlyBonus || 0,
    renewalDate: currentUser?.renewalDate || 'Próximo mes'
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white px-4 py-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl p-6 mb-8 text-white">
        ...
        <div className="mb-4">
          <p className="text-3xl font-bold">{studentInfo.points}</p>
          <p className="text-sm opacity-75">Próximo premio: {studentInfo.nextReward} pts</p>
        </div>
        <div className="text-sm">
          <p className="opacity-75">Próximo bono mensual: {studentInfo.monthlyBonus} pts</p>
          <p className="opacity-75">Renovación: {studentInfo.renewalDate}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        ...
        <button 
          className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 col-span-2"
          onClick={() => handleActionClick('/estudiante/resources/social-emotional-actions')}
        >
          <div className="flex items-center justify-center w-12 h-12 bg-pink-100 rounded-full mx-auto mb-2">
            <Heart className="w-6 h-6 text-pink-600" />
          </div>
          <span className="block text-sm font-medium text-gray-800">Acciones Socioemocionales</span>
        </button>
      </div>
