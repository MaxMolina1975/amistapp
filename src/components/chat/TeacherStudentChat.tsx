...
import { Search, Users, UserCircle, Filter, List } from 'lucide-react';
import { useAuth } from '../../lib/context/AuthContext';
import { useChat } from '../../lib/hooks/useChat';
import ChatBox from './ChatBox';
import { ChatUser, ChatAttachment } from '../../lib/types/chat';
...
const TeacherStudentChat: React.FC<TeacherStudentChatProps> = ({ 
  ...
  // Filtrar conversaciones para mostrar solo estudiantes
  const studentConversations = conversations.filter(conversation => {
    // Filtrar por término de búsqueda
    const matchesSearch = searchTerm === '' || conversation.participants.some(participant => 
      participant.name.toLowerCase().includes(searchTerm.toLowerCase())
    );
    
    // Solo mostrar conversaciones con estudiantes
    const hasStudent = conversation.participants.some(participant => 
      participant.id !== currentUser?.id && participant.role === 'student'
    );
    
    return matchesSearch && hasStudent;
  });
  
  // Efecto para iniciar conversación con un estudiante específico
  useEffect(() => {
    if (initialStudentId && currentUser && !loading) {
      // Buscar si ya existe una conversación con este estudiante
      const existingConversation = conversations.find(conv => 
        conv.participants.some(p => p.id === initialStudentId)
      );
      
      if (existingConversation) {
        selectConversation(existingConversation.id);
      } else {
        // Buscar el estudiante para iniciar una nueva conversación
        handleSearch(initialStudentId.toString());
      }
    }
  }, [initialStudentId, currentUser, loading, conversations]);
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      ...
      const studentResults = results.filter(user => user.role === 'student');
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        ...
        {studentConversations.length > 0 ? (
          <ul className="divide-y divide-gray-100 max-h-60 overflow-y-auto">
            {studentConversations.slice(0, 5).map(conversation => {
                ...
                <li key={conversation.id} className="p-2 hover:bg-gray-50 cursor-pointer" onClick={() => openFullChat(conversation.id)}>
                  <div className="flex items-center space-x-3">
                    <div className="flex-shrink-0">
                      {student.photoURL ? (
                        <img src={student.photoURL} alt={student.name} className="h-8 w-8 rounded-full" />
                      ) : (
          ...
          <div className="p-4 text-center text-gray-500">
            {loading ? 'Cargando...' : 'No hay conversaciones con estudiantes'}
          </div>
        )}
      </div>
    );
  ...
  // Modo completo
  return (
    <div className={`h-full bg-white rounded-lg shadow overflow-hidden flex ${compact ? 'max-h-[600px]' : 'h-[calc(100vh-12rem)]'}`}>
      {/* Lista de conversaciones */}
      <div className={`w-1/3 border-r ${selectedConversation && compact ? 'hidden sm:block' : ''}`}>
        <div className="p-3 border-b">
          <h2 className="font-medium text-gray-800 mb-2">Mensajes con estudiantes</h2>
          ...
          <div className="mt-2 flex items-center text-sm space-x-1">
            <button 
              className={`px-2 py-1 rounded-md ${showStudents ? 'bg-blue-100 text-blue-800' : 'hover:bg-gray-100'}`}
          ...
          <ul className="divide-y divide-gray-100 overflow-y-auto" style={{ maxHeight: compact ? '400px' : 'calc(100vh - 16rem)' }}>
            {studentConversations.length > 0 ? (
                ...
                return (
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      <div className="flex-shrink-0">
                        {student.photoURL ? (
                          <img src={student.photoURL} alt={student.name} className="h-10 w-10 rounded-full" />
                        ) : (
              ...
              <li className="p-4 text-center text-gray-500">
                {loading ? 'Cargando conversaciones...' : 'No hay conversaciones con estudiantes'}
              </li>
            )}
          </ul>
        )}
      ...
      {/* Área de chat */}
      <div className={`flex-1 ${!selectedConversation && compact ? 'hidden sm:block' : ''}`}>
        {selectedConversation ? (
          <ChatBox
            messages={messages}
          ...
          <div className="h-full flex flex-col items-center justify-center p-4 bg-gray-50">
            ...
            <h3 className="text-lg font-medium text-gray-700 mb-2">Selecciona un estudiante</h3>
            <p className="text-gray-500 text-center max-w-md">
              Selecciona una conversación existente o busca un estudiante para iniciar una nueva conversación.
            </p>
          </div>
        )}
      </div>
