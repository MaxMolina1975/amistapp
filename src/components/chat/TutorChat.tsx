...
import { Search, User, UserCircle, School, Bookmark, BookOpen } from 'lucide-react';
...
const TutorChat: React.FC<TutorChatProps> = ({ 
  ...
  const filteredConversations = conversations.filter(conversation => {
    ...
    // Filtrar por tipo de usuario (estudiante o docente)
    const matchesRole = conversation.participants.some(participant => 
      participant.id !== currentUser?.id && participant.role === (activeTab === 'students' ? 'student' : 'teacher')
    );
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      ...
      const filteredResults = results.filter(user => 
        user.role === (activeTab === 'students' ? 'student' : 'teacher')
      );
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="p-3 border-b flex justify-between items-center">
          <h3 className="font-medium text-gray-800 flex items-center">
            ...
            {activeTab === 'students' ? 'Mis Estudiantes' : 'Docentes'}
          </h3>
          
          <div className="flex space-x-2">
            <button 
              className={`px-2 py-1 text-xs rounded ${activeTab === 'students' ? 'bg-blue-100 text-blue-700' : 'text-gray-500 hover:bg-gray-100'}`}
        ...
        {filteredConversations.length > 0 ? (
          ...
          <div className="p-4 text-center text-gray-500">
            {loading ? 'Cargando...' : `No hay conversaciones con ${activeTab === 'students' ? 'estudiantes' : 'docentes'}`}
          </div>
        )}
      </div>
    );
      ...
      <div className={`w-1/3 border-r ${selectedConversation && compact ? 'hidden sm:block' : ''}`}>
        <div className="p-3 border-b">
          ...
          <div className="relative">
            <input
              type="text"
              placeholder={`Buscar ${activeTab === 'students' ? 'estudiante' : 'docente'}...`}
          ...
          <div className="mt-2 flex items-center text-sm space-x-1">
            <button 
              className={`px-2 py-1 rounded-md ${activeTab === 'students' ? 'bg-blue-100 text-blue-800' : 'hover:bg-gray-100'}`}
              onClick={() => setActiveTab('students')}
            >
          ...
          <ul className="divide-y divide-gray-100 overflow-y-auto" style={{ maxHeight: compact ? '400px' : 'calc(100vh - 16rem)' }}>
            {filteredConversations.length > 0 ? (
              ...
              <li className="p-4 text-center text-gray-500">
                {loading ? 'Cargando conversaciones...' : `No hay conversaciones con ${activeTab === 'students' ? 'estudiantes' : 'docentes'}`}
              </li>
            )}
          </ul>
        )}
